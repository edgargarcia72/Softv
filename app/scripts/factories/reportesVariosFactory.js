...
angular.module('softvApp')
	.factory('reportesVariosFactory', function ($http, $q, globalService, $localStorage) { //, sessionFactory) {
		...
		var paths = {
			...
			mostrarServicio: '/compania/GetServiciosList', //Servicios Internet, Digital, TV Cable
		...
		factory.creaReporte = function (reporteSeleccionado, servSelec, clv_usuario, elOrden, laBaja, OtrosFiltrosXml, distribuidoresXML, 
			CompaniasXml, CiudadesXml, CalleXml, LocalidadesXml, ColoniaXml, ServiciosXml, PeriodosXml, TipoClientesXml, localidadF, coloniaF, calleF) {			
			var rutaServicio;
     		...
     		if (servSelec === 1)
                {   //Contrato | Colonia y Calle; Orden = 1 | 2                   
                        ...
                        else if ((reporteSeleccionado === 6) || (reporteSeleccionado === 8) || (reporteSeleccionado === 9) || (reporteSeleccionado === 11))//6 contrataciones, 8 instalaciones, 9 cancelaciones, 11 fueras de área
                        {
                            if (elOrden === 1) //Contrato
                            {
                                rutaServicio = paths.ReporteInt_5; //url = "ReportesVarios/Reporte_Internet_5";  //url = 'ReportesVarios/Reporte_Internet_5'; 
                            }
                             else if (elOrden === 2) //Colonia y calle
                            {     
		...
		factory.mostrarServicio = function (clv_usuario, clvTipServ, Companias, banderaTodoSeleccionado) {

			var deferred = $q.defer();
			var Parametros = {
				'clv_usuario': clv_usuario,
				'clvTipServ': clvTipServ,
				'Companias': Companias,
				'banderaTodoSeleccionado': banderaTodoSeleccionado
			};
			var config = {
				headers: {
					'Authorization': $localStorage.currentUser.token
			...
			$http.post(globalService.getUrl() + paths.mostrarServicio, JSON.stringify(Parametros), config).then(function (response) {
				deferred.resolve(response.data);
			}).catch(function (response) {
				deferred.reject(response.data);
			});
		...
		factory.mostrarLocalidadByCiudad = function (clv_usuario, Companias, Ciudades, Estados) {
			var deferred = $q.defer();
			var Parametros = {
				'clv_usuario': clv_usuario,
				'Companias':Companias,
		...
		factory.mostrarColonia = function (clv_usuario, banderaLocalidad, Companias, Estados, Ciudades, Localidades) {
